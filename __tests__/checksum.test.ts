...
describe('verifyChecksum', () => {
	...
	it('uses checksums-darwin.txt first for darwin targets', async () => {
		...
		await expect(
			verifyChecksum(archivePath, assetName, 'v0.43.114', '0.43.114', target, 'Bearer token')
	...
	it('falls back to checksums.txt when checksums-darwin.txt is missing', async () => {
		...
		await expect(
			verifyChecksum(archivePath, assetName, 'v0.43.81', '0.43.81', target, undefined)
		).resolves.toBeUndefined()
	...
	it('throws on a hash mismatch with expected and actual in the message', async () => {
		...
		await expect(
			verifyChecksum(archivePath, assetName, 'v0.43.114', '0.43.114', target, undefined)
		).rejects.toThrow(
	...
	it('verifies a provided checksum without downloading checksum files', async () => {
		...
		const assetName = 'cli_0.43.114_linux_amd64.tar.gz'
		await expect(
			verifyChecksum(archivePath, assetName, 'v0.43.114', '0.43.114', target, undefined, archiveSha)
		).resolves.toBeUndefined()
	...
	it('throws when the provided checksum does not match', async () => {
		...
		await expect(
			verifyChecksum(archivePath, assetName, 'v0.43.114', '0.43.114', target, undefined, wrong)
		).rejects.toThrow(
