...
import { getAssetName, getDownloadUrl, type Target } from './platform.js'
...
function getArchiveCacheDir(
  ...
  return path.join(
    runnerTemp,
    `infisical-cli-archive-${semver}-${target.platform}-${target.arch}`
  )
}
...
export async function install(
  ...
  const cachedArchivePath = archiveDir
    ? path.join(archiveDir, assetName)
    : undefined
